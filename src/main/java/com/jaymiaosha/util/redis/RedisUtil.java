package com.jaymiaosha.util.redis;
...
//            pool = new JedisPool(config, ymlProperties.getHost(),
//                    Integer.parseInt(ymlProperties.getPort()), 100000);
        }
    }
    ...
    public static <T> List<T> getList(String key) {
        ...
        try {
            ...
            if (exists(key)) {
                ...
                return (List<T>) ObjectTranscoder.deserialize(in);
            }
        } catch (Exception e) {
            LOGGER.error(e.getMessage());
        } finally {
    ...
    public static String setObject(String key, Object object) {
        ...
        try {
            jedis = pool.getResource();
            return jedis.set(key.getBytes(), ObjectTranscoder.serialize(object));
        } catch (Exception e) {
            LOGGER.error(e.getMessage());
            return "0";
        } finally {
    ...
    public static <T> Map<String, T> getMap(String key) {
        ...
        try {
            ...
            if (exists(key)) {
                byte[] in = jedis.get(key.getBytes());
                return (Map<String, T>) ObjectTranscoder.deserialize(in);
            }
        } catch (Exception e) {
            LOGGER.error(e.getMessage());
        } finally {
    ...
    public static void batchDel(String pre_str) {
        ...
        try {
            ...
            if (null != set && set.size() > 0) {
                tr = jedis.multi();
                tr.del(set.toArray(new String[set.size()]));
                tr.exec();
            }
        } catch (Exception e) {
            tr.discard();
    ...
    public static Long zadd(String key, Map<String, Double> score) {
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
            res = jedis.zadd(key, score);
        } catch (Exception e) {
    ...
    /**
     * 通过key返回指定score内zset中的value</p> 成员的位置按分数值递增(从小到大)来排序。
     *
     * @param key
     * @param start
     * @param end
